...
public class MageSkills : MonoBehaviour
{
    ...
    [SerializeField] private Transform defaultVfxSpawn;     // v? trÌ spawn VFX m?c d?nh (hand)
    ...
    [SerializeField] private Transform weaponSheathPoint;    // di?m nÈm weapon ra sau
    ...
    private void Start()
    {
        // L?ng nghe s? ki?n d?i vu khÌ t? khi b?t d?u game
        ...
        // C?p nh?t tr?ng th·i b?t/t?t l?n d?u tiÍn
        RefreshActiveForCurrentWeapon();
    }

    private void OnDestroy()
    {
        // Ch? h?y l?ng nghe khi c?c Player n‡y ho‡n to‡n b? xÛa kh?i map
        var wc = GetComponent<WeaponController>();
    ...
    private void OnEnable()
    {
        ...
        RefreshActiveForCurrentWeapon(); // NEW: t·ª± b·∫≠t/t·∫Øt theo weapon hi·ªán t·∫°i
    }
    ...
    // NEW: ch? b?t script khi dang c?m vu khÌ Mage
    private void RefreshActiveForCurrentWeapon()
    {
        var w = equipment != null ? equipment.GetCurrentWeapon() : null;
        // FIX: KhÙng t? disable script n?a d? h‡m Update (Failsafe) luÙn du?c ch?y!
    ...
    // Mage gi·ªù d√πng logic VFX nh∆∞ Sword/Axe - lo·∫°i b·ªè projectile methods th·ª´a
    ...
    // CHU·∫®N H√ìA: ch·ªâ d√πng AE_PlaySkillVFXByEvent ƒë·ªÉ spawn VFX
    public void AE_PlaySkillVFXByEvent(int eventIndex)
    {
        ...
        if (ability.isFollowPlayer)
        {
            ...
            // ShieldActivate trÍn prefab t? qu?n l˝ NavMeshObstacle + ch?n damage
    ...
    // NEW: overload khÙng tham s? ñ an to‡n cho clip cu g?i AE_PlaySkillVFXByEvent()
    ...
    /// <summary>
    /// ƒê∆∞·ª£c g·ªçi b·ªüi NetworkCombatSync tr√™n Proxy ƒë·ªÉ ch·∫°y Timeline VFX.
    /// Kh√¥ng ti√™u t·ªën Cooldown, ch·ªâ ph√°t h√¨nh ·∫£nh.
    /// </summary>
